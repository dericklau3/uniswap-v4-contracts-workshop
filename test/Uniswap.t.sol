...
contract UniswapTest is Test {
    ...
    function testIncreaseLiquidity() public {
        vm.startBroadcast(account);
        (PoolKey memory poolKey,, uint256 tokenId) = _addLiquidityV4(address(weth), address(usdt), 10e18, 30000e18);

        uint128 liquidityBefore = positionManager.getPositionLiquidity(tokenId);
        assertGt(liquidityBefore, 0, "initial liquidity should be minted");

        uint256 tokenAAmount = 5e18;
        uint256 tokenBAmount = 15000e18;
        (uint256 amount0, uint256 amount1) = address(weth) < address(usdt)
            ? (tokenAAmount, tokenBAmount)
            : (tokenBAmount, tokenAAmount);

        uint160 sqrtPriceX96 = calculateSqrtPriceX96(address(weth), address(usdt), tokenAAmount, tokenBAmount);
        (int24 tickLower, int24 tickUpper) = calculateAMMRange(60);
        uint128 addedLiquidity = getLiquidityForAmounts(sqrtPriceX96, tickLower, tickUpper, amount0, amount1);

        bytes memory actions = abi.encodePacked(uint8(Actions.INCREASE_LIQUIDITY), uint8(Actions.SETTLE_PAIR));
        bytes[] memory params = new bytes[](2);
        params[0] = abi.encode(tokenId, addedLiquidity, uint128(amount0), uint128(amount1), new bytes(0));
        ...
        assertEq(
            ...
            liquidityBefore + addedLiquidity,
    ...
    function testV4SwapExactInSingle() public {
        vm.startBroadcast(account);
        _addLiquidityV4(address(weth), address(usdt), 10e18, 30000e18);
        ...
        // 判断weth 是不是 token0
        // zeroForOne: true 表示 token0 换 token1， false 表示 token1 换 token0
        bool zeroForOne = token0 == address(weth) ? false : true;
        ...
        // zeroForOne: true token0 = usdt. token1 = weth -> usdt 换 weth
        if (zeroForOne) {
            // 支付输入token
            ...
            // zeroForOne: false token0 = weth. token1 = usdt -> usdt 换 weth
            // 支付输入token
    ...
    function testV4SwapExactOutSingle() public {
        ...
        // 3. Single token pair exact output swap (ETH -> USDT)
        console.log("==== Single token pair exact output swap (ETH -> USDT) ====");
        ...
        // zerForOne : true token0 = weth, token1 = usdt  -> weth 换 usdt
        // zeroForOne : false token0 = usdt, token1 = weth -> weth 换 usdt
        bool zeroForOne = address(weth) < address(usdt) ? true : false;
        ...
        // 最多支付 weth 的 amountInMax
        paramsExactOutSingle[1] = abi.encode(Currency.wrap(address(weth)), amountInETHMax);

        // 拿走精确输出 usdt
        paramsExactOutSingle[2] = abi.encode(Currency.wrap(address(usdt)), 2000e18);
    ...
    function testV4SwapExactOut() public {
        ...
        // Multi token pair exact output swap (ETH -> USDT -> BTC)
        console.log("==== Multi token pair exact output swap (ETH -> USDT -> BTC) ====");
        ...
        // 最多支付最初输入币 ETH
        paramsExactOut[1] = abi.encode(Currency.wrap(address(weth)), amountInETHMax);

        // 拿最终输出币 BTC
        paramsExactOut[2] = abi.encode(currencyOutExactOut, 0.01e18);
    ...
    function calculateAMMRange(int24 tickSpacing) internal pure returns (int24 tickLower, int24 tickUpper) {
